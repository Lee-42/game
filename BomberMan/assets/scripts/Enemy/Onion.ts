...
@ccclass("Onion")
export class Onion extends Enemy {
  start() {
    super.start();
    console.log("Onion started");
  ...
  onBeginContact(
    ...
    // preserve Enemy death logic
    super.onBeginContact(selfCollider, otherCollider, contact);

    if (this._isDead) return;

    console.log(`Onion collided with ${otherCollider.node.name}`);
  ...
  playAnimation(dir: Vec2) {
    ...
    if (dir.x < 0) {
      this._anim.play("onion-walk-left");
    } else if (dir.x > 0) {
      this._anim.play("onion-walk-right");
    } else {
      // Vertical movement: keep previous side animation.
      const state = this._anim.getState("onion-walk-left");
      const stateRight = this._anim.getState("onion-walk-right");
