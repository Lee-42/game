...
export class BattleField extends Component {
  ...
  /**
   * get next action actor, sorted by speed, the fastest actor will be the next action actor
   * @param targets
   * @returns
   */
  ...
  /**
   * get next defender, sorted by taunt, the highest taunt actor will be the next defender
   * @param targets
   * @returns
   */
  ...
  /**
   * check if the actor is from left
   * @param target
   * @returns
   */
  ...
  /**
   * check if the actor can cast skill, the rage cost of the skill must be less than the rage of the actor
   * @param caster
   * @returns
   */
  ...
  battleLoop(targets: Mediator[]) {
    ...
    let defender: Mediator = isAttackerLeft
      ? this.getNextDefender(this.rightFishes)
      : this.getNextDefender(this.leftFishes);

    let commandQueue = new CommandQueue();
    const startPosition = new Vec3(
      attacker.node.worldPosition.x,
      attacker.node.worldPosition.y,
      attacker.node.worldPosition.z
    );
    let duration = 0;
    const damageNode = instantiate(this.damagePrefab);

    // if the actor can cast skill, cast skill
    if (this.isCanSkill(attacker)) {
      ...
      // if the skill should not move, cast skill
      if (skillCfg.shouldMove == 1) {
        const skillCommand = new MainSkillCastCommand(
          attacker,
          targets,
          id,
          damageNode,
        ...
        // if the skill should move, cast skill and move to the target, then move back to the original position
        const skillCommand = new MainSkillCastCommand(
          attacker,
          targets,
          id,
        ...
        const targePostion = new Vec3(
          moveTarget.node.worldPosition.x +
            moveTarget.getModelWidth() * moveTarget.isReverse,
          moveTarget.node.worldPosition.y,
          moveTarget.node.worldPosition.z
        );
        const move = new MoveCommand(
          attacker,
          targePostion,
          Constants.MoveDuration
        );
        const moveBack = new MoveCommand(
          attacker,
          startPosition,
          Constants.MoveDuration
        );
      ...
      // if the actor can not cast skill, attack the defender
      let attackType = attacker.actor.cfg.attackType;
      if (attackType == AttackType.Healing) {
      } else if (attackType == AttackType.MeleeAttack) {
        const targePostion = new Vec3(
          defender.node.worldPosition.x +
            defender.getModelWidth() * defender.isReverse,
          defender.node.worldPosition.y,
          defender.node.worldPosition.z
        );
        const move = new MoveCommand(
          attacker,
          targePostion,
        ...
        if (bullet) {
          ...
          const bulletFireCommnad = new BulletFireCommnad(
            bullet,
            attacker,
            defender,
            Constants.ShootingDuration,
            damageNode
          );
      ...
      if (this.checkGameover()) {
        ...
        if (rightAliveFishes && rightAliveFishes.length == 0) {
          ...
          targets = targets.filter(
            (fish) => fish.actor.uuId != attacker.actor.uuId
          );
          if (targets.length > 0) {
  ...
  update(deltaTime: number) {
    if (
      this._allPrefabCount == this._prefabLoadingCount &&
      this._isBattleBegin == false
    ) {
